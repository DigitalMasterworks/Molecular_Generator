...
# Zero-arg; needs molecules.csv next to it.
...
def read_atomic_masses(filename="molecules.csv", symbol_col="NAME", mass_col="MW", delimiter=','):
    atoms = []
    with open(filename, newline='') as f:
        ...
        for row in r:
            try:
                sym = row[symbol_col]
                mw_g_per_mol = float(row[mass_col])  # g/mol == u
                mass_mev = mw_g_per_mol * 931.49410242
